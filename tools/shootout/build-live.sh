...
#
# Compiles antiphon-ffi to wasm32 (+simd128 — same artifact the parity check uses), drops it at
...
# C ABI must stay identical across engines (edit antiphon-dsp internals, not the antiphon-ffi surface).
set -euo pipefail
...
echo "building antiphon-ffi wasm for engine '$ID'…"
RUSTFLAGS="-C target-feature=+simd128" \
  cargo build -p antiphon-ffi --release --target wasm32-unknown-unknown

mkdir -p out/shootout/wasm
cp target/wasm32-unknown-unknown/release/antiphon_ffi.wasm "out/shootout/wasm/$ID.wasm"
