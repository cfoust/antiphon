...
bit-exact no-ops at 0 — see `docs/internal/conventions.md` before touching them.
...
- **One coordinate frame.** `antiphon-dsp` is the single source of truth (right-handed:
  ...
  Read `docs/internal/conventions.md` before touching anything geometry/ITD/pose related — the ITD sign
...
- Native app needs only Command Line Tools `swiftc` (no Xcode, no SwiftPM). If `swiftc` errors
  about a duplicate `SwiftBridging` module, disable the stale CLT modulemap once (see `docs/internal/build.md`).
...
- `docs/` is the PUBLISHED Docusaurus site (antiphon.dev/docs; `just docs` to serve, content
  under `docs/docs/` + `docs/blog/`, translations in `docs/i18n/`). Engineering notes live
  unrendered in `docs/internal/`.
- `docs/internal/conventions.md` — coordinate frame, azimuth direction, ITD sign, pose-from-tracker. Read first.
- `docs/internal/build.md` — full build/toolchain details.
- `docs/internal/web.md` — web app structure (worklet, `src/audio/`, bridge live mode).
- `docs/internal/sofa.md` — measured SOFA importer (strict fidelity upgrade over the analytic placeholder).
