...
bit-exact no-ops at 0; the zero `facing` vector means omnidirectional, and image sources
mirror the facing axis per bounce parity.
...
- **One coordinate frame.** `antiphon-dsp` is the single source of truth (right-handed:
  ...
  The pinned facts — they trivially flip left/right if you guess: azimuth `0` front,
  `+π/2` LEFT; unit vector for `(az, el)` is `[-cos(el)·sin(az), sin(el), -cos(el)·cos(az)]`
  (defined once in `antiphon-assets::AssetBuilder::push_direction`); ITD is fractional
  samples with `itd > 0` = source toward the LEFT = RIGHT ear delayed; ears sit
  `EAR_HEIGHT_M` (1.6 m) above the floor, world origin at the nominal ear position.
...
- Native app needs only Command Line Tools `swiftc` (no Xcode, no SwiftPM). If `swiftc` errors
  about a duplicate `SwiftBridging` module, disable the stale CLT modulemap once:
...
- `docs/` is the PUBLISHED Docusaurus site (antiphon.dev/docs; `just docs` to serve, content
  under `docs/docs/` + `docs/blog/`, translations in `docs/i18n/`).
- `plugins/README.md` — the antiphond wire protocol + per-agent adapter support matrix.
