...
**[下载 macOS 版 Antiphon](https://github.com/cfoust/antiphon/releases/latest/download/Antiphon-macOS.zip)**

解压后把 `Antiphon.app` 拖入 `/Applications`。

:::note Gatekeeper
发布版本尚未使用 Apple Developer ID 进行公证。第一次启动时，macOS 会礼貌地拒绝；清除隔离标记后再打开即可：
...
密钥存储在 `~/.antiphon/config.json`（权限 `0600`，仅限本机）。环境变量 `ELEVENLABS_API_KEY` 和 `OPENAI_API_KEY` 可作为后备。
