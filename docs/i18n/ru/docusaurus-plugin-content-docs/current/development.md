...
2. **Одна система координат.** Геометрией владеет DSP-крейт (правая система, фронт = −z,
   азимут — в сторону +left). Хосты конвертируют на своей границе. Закреплённые
   факты (азимут в сторону +left, знак ITD) живут в `CLAUDE.md` репозитория — если
   действовать наугад, левое и правое поменяются местами.
