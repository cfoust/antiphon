...
//! drives `antiphon-dsp` deterministically, block by block, exactly as a real-time host
//! would, and writes 48 kHz/16-bit stereo WAVs you can listen to on headphones.
//!
//! Usage:
//!   antiphon-render [asset.antiphon] [out_dir]
...
fn main() {
    ...
    // `suite <wav_dir> <out_dir>`: A/B test matrix for the Antiphon application.
    ...
    if std::env::args().nth(1).as_deref() == Some("voices") {
        ...
        run_voices(
            a.get(2).map(|s| s.as_str()).unwrap_or("out/voices_src"),
            a.get(3).map(|s| s.as_str()).unwrap_or("assets/baked/antiphon-ari.antiphon"),
    ...
    if std::env::args().nth(1).as_deref() == Some("scenario") {
        ...
        run_scenario(
            ...
            a.get(3).map(|s| s.as_str()).unwrap_or("assets/baked/antiphon-kemar.antiphon"),
    ...
    if std::env::args().nth(1).as_deref() == Some("shootout") {
        let a: Vec<String> = std::env::args().collect();
        let asset = a.get(2).cloned().unwrap_or_else(|| "assets/baked/antiphon-default.antiphon".into());
    ...
    if std::env::args().nth(1).as_deref() == Some("nearfield") {
        ...
        run_nearfield(
            ...
            a.get(9).map(|s| s.as_str()).unwrap_or("assets/baked/antiphon-default.antiphon"),
        );
        return;
    }
    ...
    let asset = AntiphonAsset::parse(&bytes).expect("parse asset");
    println!(
    ...
    // --- Scene 7: five voices on a frontal arc (the "antiphon" layout) ---
    {
...
/// A/B suite for the Antiphon application: full antiphon scenes (HRTF × room) and isolated
...
fn run_suite(wav_dir: &str, out_dir: &str) {
    ...
    let load = |p: &str| -> Option<AntiphonAsset> {
        std::fs::read(p).ok().and_then(|b| AntiphonAsset::parse(&b).ok())
    };
    let ari = load("assets/baked/antiphon-ari.antiphon");
    let kemar = load("assets/baked/antiphon-kemar.antiphon");
    let default = load("assets/baked/antiphon-default.antiphon");
    fn pick<'a>(a: &'a Option<AntiphonAsset>, d: &'a Option<AntiphonAsset>) -> Option<&'a AntiphonAsset> {
        a.as_ref().or(d.as_ref())
    }

    println!("Antiphon A/B suite -> {}/", out_dir);
    println!("  assets: ari={} kemar={} default={}", ari.is_some(), kemar.is_some(), default.is_some());

    // ---- full antiphon scenes: 6 voices on a front arc, head pans, two agents emit pings ----
    if let Some(a) = pick(&ari, &default) {
        antiphon_scene(a, out_dir, "scene_ari_hall", "hall", &voices, true);
        antiphon_scene(a, out_dir, "scene_ari_room", "room", &voices, true);
        antiphon_scene(a, out_dir, "scene_ari_hallconv", "hall_conv", &voices, true);
    }
    if let Some(a) = pick(&kemar, &default) {
        antiphon_scene(a, out_dir, "scene_kemar_hall", "hall", &voices, true);
    }
...
fn room_idx(a: &AntiphonAsset, want: &str) -> usize {
    a.rooms.iter().position(|r| r.name == want).unwrap_or(0)
}

/// Full antiphon: 6 voices on a ±90° front arc, head pans ±70°, the faced voice opens up
...
fn ping_test(asset: &AntiphonAsset, out_dir: &str, name: &str, room: &str, rich: bool, bearing_deg: f32) {
...
Antiphon A/B suite — listen on headphones.
...
fn run_bench(asset_path: &str) {
    let bytes = std::fs::read(asset_path).expect("asset");
    let asset = AntiphonAsset::parse(&bytes).unwrap();
...
fn run_attention(out_path: &str, agents: u32, build_min: f32, dur: f32, room: &str) {
    let bytes = std::fs::read("assets/baked/antiphon-default.antiphon").expect("read asset");
    let asset = AntiphonAsset::parse(&bytes).expect("parse asset");
    let room_names: Vec<String> = asset.rooms.iter().map(|r| r.name.clone()).collect();
    let mut r = Renderer::new(&asset, SR, 1, BLOCK);
