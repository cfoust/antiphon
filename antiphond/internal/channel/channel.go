// Package channel is `antiphond channel`: the per-session MCP server Claude
...
Messages arriving in <channel source="antiphon"> tags are the user speaking to you
...
// New builds a channel. hubURL "" uses ANTIPHON_HUB or the default port.
func New(hubURL string) *Channel {
	if hubURL == "" {
		hubURL = os.Getenv("ANTIPHON_HUB")
	}
	if hubURL == "" {
...
func (c *Channel) dispatch(req rpcRequest) {
	...
	case "initialize":
		c.reply(req.ID, map[string]any{
			...
			"serverInfo":   map[string]any{"name": "antiphon", "version": "0.1.0"},
	...
	for {
		...
		for {
			...
			case "seat":
				...
				c.hubMu.Unlock()
			case "channel":
				// the user spoke to this agent from the antiphon
				c.hubMu.Lock()
				seat := c.seat
				c.hubMu.Unlock()
...
func sessionID() string {
	for _, k := range []string{"ANTIPHON_SESSION", "CLAUDE_SESSION_ID", "CLAUDE_CODE_SESSION_ID"} {
		if v := os.Getenv(k); v != "" {
			return v
		}
	}
